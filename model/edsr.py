...
def res_block(x_in, filters, scaling, ks):
    x = Conv2D(filters, ks, padding='same', activation='relu')(x_in)
    x = Conv2D(filters, ks, padding='same')(x)
    if scaling:
...
def upsample(x, scale, num_filters, ks):
